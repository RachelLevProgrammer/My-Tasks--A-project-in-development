  ...
  const handleSaveTask = async () => {
    ...
    if (!IsEdit) {
      ...
      try {
        ...
        const response = await axios.put(`http://localhost:8080/tasks/updateTask/${taskData._id}`, dataToSend, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
        ...
        if (response.status === 200 || response.status === 201) {
          setIsEdit(false);
          const updatedCalendar = [...calendar.filter(task => task._id !== taskData._id), response.data.newTask];
        ...
        alert(isRTL ? `עדכון המשימה נכשל: ${error?.response?.data?.message || error.message}` : `Task update failed: ${error?.response?.data?.message || error.message}`);
      }
    }
      }
